			...
			<el-form class="center-form-pv" :style='{"padding":"20px","boxShadow":"0 1px 10px 1px rgba(115,108,203,.1)","margin":"0px","overflow":"hidden","borderRadius":"3px","background":"#fff","fontSize":"inherit"}' :inline="true" :model="searchForm">
				<el-row :style='{"width":"100%","padding":"10px","fontSize":"inherit","textAlign":"left","background":"#fff","display":"block"}' >
					<div :style='{"margin":"0 10px 0 0","fontSize":"inherit","display":"inline-block"}'>
						<label :style='{"margin":"0 10px 0 0","color":"inherit","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">农场名称</label>
						<el-input v-model="searchForm.nongchangmingcheng" placeholder="农场名称" @keydown.enter.native="search()" clearable></el-input>
					</div>
				...
				<el-row class="actions" :style='{"margin":"20px 0 5px","color":"#fff","flexWrap":"wrap","textAlign":"left","flexDirection":"row","display":"flex","width":"100%","fontSize":"inherit"}'>
					<el-button class="add" v-if="isAuth('nongchangxinxi','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia13" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","display":"none"}'></span>
						增加
					</el-button>
					<el-button class="del" v-if="isAuth('nongchangxinxi','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
			...
			<div :style='{"border":"0px solid #eee","padding":"0","boxShadow":"0 1px 10px 1px rgba(115,108,203,.1)","margin":"30px 0 0","borderRadius":"0px","background":"rgba(255,255,255,.9)","width":"100%"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column :resizable='true' :sortable='true'  
						prop="nongchangmingcheng"
						label="农场名称">
						<template slot-scope="scope">
							{{scope.row.nongchangmingcheng}}
						</template>
					</el-table-column>
					<!-- 无 -->
					<el-table-column :resizable='true' :sortable='true' prop="nongchangtupian" width="200" label="农场图片">
						<template slot-scope="scope">
							<div v-if="scope.row.nongchangtupian">
								<img v-if="scope.row.nongchangtupian.substring(0,4)=='http'" :src="scope.row.nongchangtupian.split(',')[0]" width="100" height="100">
								<img v-else :src="$base.url+scope.row.nongchangtupian.split(',')[0]" width="100" height="100">
							</div>
							<div v-else>无图片</div>
						</template>
					</el-table-column>
					<el-table-column :resizable='true' :sortable='true'  
						prop="nongchangmianji"
						label="农场面积/亩">
						<template slot-scope="scope">
							{{scope.row.nongchangmianji}}
						</template>
					</el-table-column>
					<el-table-column :resizable='true' :sortable='true'  
						prop="turangzhiliang"
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('nongchangxinxi','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-chakan14" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","display":"none","height":"40px"}'></span>
								详情
							</el-button>
							<el-button class="edit" v-if=" isAuth('nongchangxinxi','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
							...
							<el-button class="del" v-if="isAuth('nongchangxinxi','删除') " type="primary" @click="deleteHandler(scope.row.id )">
...
import AddOrUpdate from "./add-or-update";
    ...
    getDataList() {
      ...
      let params = {
        ...
        order: 'desc',
           ...
           if(this.searchForm.nongchangmingcheng!='' && this.searchForm.nongchangmingcheng!=undefined){
            params['nongchangmingcheng'] = '%' + this.searchForm.nongchangmingcheng + '%'
          }
			this.$http({
	...
	nongchangxinxistatusChange(e,row){
		if(row.status==0){
			row.passwordwrongnum = 0
		}
		this.$http({
			url:'nongchangxinxi/update',
			method:'post',
			data:row
		}).then(res=>{
			if(row.status==1){
